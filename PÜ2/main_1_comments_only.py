# UC 2.0

#%% VORBEDINGUNG: UC 1.0 Erstellen der Daten
'''
Daten sind vollständig vorhanden, dh.es wurde am Ergometer ein Leistungstest gemacht und aufgezeichnet
sowie die Person im System angelegt(ID) 
'''
#%% UC 2.1 Einlesen der Daten
'''
Die Daten werden eingelesen 
''' 
#%% UC 2.2 Vorverarbeiten der Daten
'''
Daten werden vorverarbeitet 
'''
#%% UC 2.3 Analysieren der Daten auf Abbruchkriterien
'''
Es wird festgestellt ob die vorverarbeiteten Daten irgendwelche Abbruchkriterien erfüllen 
'''
#%% UC 2.4 Erstellen einer Zusammenfassung
'''
Eine Zusammenfassung der Daten wird erstellt
'''
#%% UC 2.5 Visualisierung der Daten
'''
Die aufbereiteten Daten werden dargestellt 
'''
#%% UC 2.6 Manuelle Eingabe eines Abbruch-Kriteriums
'''
Diagnostiker/in erkennt Abbruchgrund und gibt diesen ein. Eingabe wird dokumentiert und mit Daten gespeichert. 
'''
#%% UC 2.7 Speichern der Daten
'''
Die Daten werden gespeichert 
'''
#%%Nachbedingung 
'''
Grafiken und Auswertungen sind gespeichert. Eingangsordner ist leer.
'''
 
